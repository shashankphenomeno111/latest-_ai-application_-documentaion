...
## 4. Training & Validation Specifications

### A. Model Training Hyperparameters
...
### B. Validation Split Mechanics
To ensure the model is evaluated on unseen data and does not simply memorize training records:
* **Temporal Cutoff**: The training cutoff is set at `time_idx.max() - 4` weeks. All historical sequences preceding this cutoff form the **Training Dataset**.
* **Holdout Validation**: The final **4 weeks (28 days)** of transactions are held out entirely as the **Validation Dataset**.
* **Epoch Monitoring**: At the end of each training epoch, the model is evaluated against the validation set to calculate the validation loss (`val_loss`), ensuring we only save the weights that generalize best to future prediction steps.

### C. Validation Loss Results & Convergence
* **Best Validation Loss (`val_loss`)**: **`464.13388`**
* **Epoch of Best Checkpoint**: **Epoch 7** (representing 8 complete training epochs, indexed 0-7, and 50,112 weight updates).
* **Convergence Behavior**: The training terminated after Epoch 12 because validation loss did not improve beyond the best score of `464.134` for 5 consecutive epochs (Early Stopping patience reached). This indicates the model successfully converged to its optimal weight state without overfitting.
* **Interpretation of the Loss Value**: In PyTorch Forecasting, `QuantileLoss` is computed by summing the differences between the predicted quantiles (10%, 50%, 90%) and the normalized target values across all time steps and groups in a validation batch. A score of `464.134` across the validation dataset represents excellent fit and stable quantile boundaries across thousands of different commodity/district time-series nodes.
